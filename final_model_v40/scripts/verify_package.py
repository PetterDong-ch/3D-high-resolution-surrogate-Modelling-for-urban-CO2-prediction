...
def sha256(path: Path) -> str:
    """Compute SHA-256 in chunks so large checkpoint files are safe to hash."""
...
def main() -> int:
    """Verify released files and optional external-data contract."""

    root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser(description="Verify the V40 reproducibility package.")
    parser.add_argument("--data-root", type=Path, help="Optional external V40 data root.")
    ...
    if args.data_root:
        ...
        print("External data not checked; pass --data-root after obtaining the dataset.")
